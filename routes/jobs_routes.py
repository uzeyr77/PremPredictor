...
def trigger_refresh_cache():
    ...
    thread = threading.Thread(target=_run_refresh_cache, daemon=True)
    thread.start()
...
def _run_update_match_data() -> None:
    try:
        cfg = current_app.config["APP_CONFIG"]
...
def trigger_update_match_data():
    ...
    thread = threading.Thread(target=_run_update_match_data, daemon=True)
    thread.start()
